...
if(!form.id) {
  ...
  form.put(route('creditnotes.update',{note: form.id}), {
      preserveScroll: true,
      preserveState: false,
      onSuccess: () => {
        form.reset()
...
<form class="commercial-document-page" :class="commercialDocumentThemeClasses" @submit.prevent>
        ...
        <div class="-mx-4 mt-8 flow-root sm:mx-0">
          <table class="min-w-full">
              ...
              <tr v-for="(item, index) in itemsWithSubTotal" :key="index" class="border-b border-gray-200">
                <td class="max-w-0 py-5 pl-4 pr-3 text-sm sm:pl-0 align-top">
                  ...
                  <div class="mt-2 truncate text-gray-500">
                          <textarea v-model="item.item.obs" :placeholder="$t('gestlab.general.labels.credit_notes.obs')" rows="2" :name="`obs-${index+1}`" :id="`obs-${index+1}`" class="ds-field min-h-20 resize-none py-2 text-sm" />
                  </div>
                </td>
            ...
            <tfoot>
              ...
              <tr>
                <th scope="row" colspan="4" class="hidden pl-4 pr-3 pt-4 text-right text-sm font-normal text-gray-500 sm:table-cell sm:pl-0">{{ $t('gestlab.general.labels.credit_notes.discount_total') }}</th>
                <th scope="row" class="pl-6 pr-3 pt-4 text-left text-sm font-normal text-gray-500 sm:hidden">{{ $t('gestlab.general.labels.credit_notes.discount_total') }}</th>
    ...
    <div class="sm:col-span-full">
      <label for="obs" class="ds-field-label">{{ $t('gestlab.general.labels.credit_notes.obs') }}</label>
      <div class="mt-2">
