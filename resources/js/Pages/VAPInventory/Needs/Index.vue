<template>
  <div class="space-y-8" :class="commercialDocumentThemeClasses">
    <ModuleHero
      ...
      title="Necessidades de laboratório e departamento"
      description="Consolide requisições de laboratórios, submeta para aprovação e transforme apenas necessidades aprovadas em pedidos de compra rastreáveis."
    >
      <template #actions>
        <Link :href="route('vap-inventory.needs.create')" class="inline-flex items-center justify-center rounded-2xl bg-primary-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-primary-600/20 transition hover:bg-primary-700 dark:bg-primary-500 dark:hover:bg-primary-400">
          Nova necessidade
        </Link>
      </template>

      <div class="mt-6 grid gap-4 md:grid-cols-4">
        <article class="rounded-2xl border border-slate-200 bg-white/70 p-4 shadow-sm dark:border-slate-700 dark:bg-slate-950/45">
          <div class="text-sm text-slate-500 dark:text-slate-400">Total</div>
          <div class="mt-2 text-2xl font-semibold text-slate-950 dark:text-white">{{ stats.total }}</div>
        </article>
        <article class="rounded-2xl border border-amber-200 bg-amber-50/80 p-4 dark:border-amber-500/30 dark:bg-amber-500/10">
          <div class="text-sm text-amber-800 dark:text-amber-200">Submetidas</div>
          <div class="mt-2 text-2xl font-semibold text-amber-700 dark:text-amber-100">{{ stats.submitted }}</div>
        </article>
        <article class="rounded-2xl border border-emerald-200 bg-emerald-50/80 p-4 dark:border-emerald-500/30 dark:bg-emerald-500/10">
          <div class="text-sm text-emerald-800 dark:text-emerald-200">Aprovadas</div>
          <div class="mt-2 text-2xl font-semibold text-emerald-700 dark:text-emerald-100">{{ stats.approved }}</div>
        </article>
        <article class="rounded-2xl border border-primary-200 bg-primary-50/80 p-4 dark:border-primary-500/30 dark:bg-primary-500/10">
          <div class="text-sm text-primary-800 dark:text-primary-200">Em aquisição</div>
          <div class="mt-2 text-2xl font-semibold text-primary-700 dark:text-primary-100">{{ stats.ordered }}</div>
        </article>
      </div>

      <div class="mt-4 grid gap-4 md:grid-cols-2">
        <article class="rounded-2xl border border-amber-200 bg-amber-50/80 p-4 dark:border-amber-500/30 dark:bg-amber-500/10">
          <div class="text-sm text-amber-800 dark:text-amber-200">Aprovadas à espera de pedido</div>
          <div class="mt-2 text-2xl font-semibold text-amber-900 dark:text-amber-100">{{ stats.awaiting_order }}</div>
        </article>
        <article class="rounded-2xl border border-rose-200 bg-rose-50/80 p-4 dark:border-rose-500/30 dark:bg-rose-500/10">
          <div class="text-sm text-rose-800 dark:text-rose-200">Procurement em atraso</div>
          <div class="mt-2 text-2xl font-semibold text-rose-900 dark:text-rose-100">{{ stats.overdue_procurement }}</div>
        </article>
      </div>
    </ModuleHero>
    ...
    <section class="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
      <ModuleCard>
        <div class="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 class="text-lg font-semibold text-slate-900 dark:text-white">Estado das necessidades</h2>
          ...
          <div class="rounded-2xl bg-slate-50 px-4 py-3 text-right dark:bg-slate-950/50">
            <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Registos monitorizados</p>
            <p class="mt-2 text-2xl font-semibold text-slate-900 dark:text-white">{{ statusOverviewTotal }}</p>
          </div>
        </div>
      ...
      <div class="grid gap-6">
        <ModuleCard>
          <div class="flex items-start justify-between gap-4">
            <div>
              <h2 class="text-lg font-semibold text-slate-900 dark:text-white">Prontidão da fila</h2>
              <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Leitura do risco de fornecedor dentro da fila de procurement.</p>
            </div>
        ...
        <ModuleCard>
          <div class="flex items-start justify-between gap-4">
            <div>
              <h2 class="text-lg font-semibold text-slate-900 dark:text-white">Pressão de procurement</h2>
              <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Tamanho da fila e concentração de necessidades urgentes ou em atraso.</p>
            </div>
          </div>
    ...
    <ModuleCard>
      <div class="grid gap-4 md:grid-cols-4">
        ...
        <div class="flex gap-2">
          <button type="button" class="flex-1 rounded-2xl border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800" @click="resetFilters">
            Limpar
          </button>
          <button type="button" class="flex-1 rounded-2xl bg-primary-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-primary-700 dark:bg-primary-500 dark:hover:bg-primary-400" @click="applyFilters">
            Filtrar
          </button>
        </div>
      </div>
    ...
    <ModuleCard title="Fila de procurement" description="Necessidades já aprovadas e ainda sem pedido de compra associado.">
      <div v-if="procurementQueue?.length" class="mb-8 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        >
          <div class="flex items-center justify-between gap-3">
            <span class="rounded-full bg-white/70 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-slate-700 dark:bg-slate-950/60 dark:text-slate-200">{{ need.reference }}</span>
          ...
          <h3 class="mt-3 text-sm font-semibold text-slate-900 dark:text-white">{{ need.department?.name }}<span v-if="need.lab"> · {{ need.lab.name }}</span></h3>
          <p class="mt-2 line-clamp-3 text-sm text-slate-600 dark:text-slate-300">{{ need.justification || 'Sem justificação adicional.' }}</p>
          <div class="mt-3 space-y-1 text-xs text-slate-600 dark:text-slate-300">
            <div>Itens: <span class="font-semibold text-slate-900 dark:text-white">{{ need.items_count }}</span></div>
            <div>Necessário até: <span class="font-semibold text-slate-900 dark:text-white">{{ formatDate(need.needed_by_date) }}</span></div>
            <div>Solicitante: <span class="font-semibold text-slate-900 dark:text-white">{{ need.requested_by?.name || '—' }}</span></div>
          </div>
          ...
          <Link :href="route('vap-inventory.needs.show', need.id)" class="mt-4 inline-flex items-center text-sm font-semibold text-cyan-700 hover:text-cyan-800 dark:text-cyan-300 dark:hover:text-cyan-200">
            Abrir necessidade
          </Link>
        </article>
      </div>
      ...
      <div v-if="needs.data.length" class="space-y-4">
        <article v-for="need in needs.data" :key="need.id" class="rounded-2xl border border-slate-200 bg-slate-50/60 p-5 dark:border-slate-800 dark:bg-slate-950/45">
          <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div class="space-y-2">
              <div class="flex flex-wrap items-center gap-2">
                <span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold uppercase tracking-wide text-slate-600 dark:bg-slate-900 dark:text-slate-300">{{ need.reference }}</span>
              ...
              <h2 class="text-lg font-semibold text-slate-900 dark:text-white">{{ need.department?.name }}<span v-if="need.lab"> · {{ need.lab.name }}</span></h2>
              <p class="text-sm leading-6 text-slate-600 dark:text-slate-300">{{ need.justification || 'Sem justificação adicional.' }}</p>
              <div class="grid gap-2 text-sm text-slate-500 dark:text-slate-400 md:grid-cols-3">
            ...
            <Link :href="route('vap-inventory.needs.show', need.id)" class="inline-flex items-center justify-center rounded-2xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
              Abrir detalhe
            </Link>
          </div>
        </article>
      </div>
      <div v-else class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-950/50 dark:text-slate-400">
...
const queueCardClass = (need) => {
  ...
  if (label === 'Próximo') {
    return 'border-cyan-200 bg-cyan-50 text-cyan-900 dark:border-cyan-500/30 dark:bg-cyan-500/10 dark:text-cyan-100'
  }

  return 'border-slate-200 bg-slate-50 text-slate-900 dark:border-slate-800 dark:bg-slate-950/45 dark:text-slate-100'
}
