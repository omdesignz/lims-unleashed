...
function createRecord() {
  router.get(route("importcertificates.create"));
}
...
function executeBulkAction(selectedAction) {
  ...
  const routeName = {
    delete: "importcertificates.destroy",
    restore: "importcertificates.restore",
  }[selectedAction];
...
<template>
  <div class="import-certificates-index space-y-6" :class="commercialDocumentThemeClasses">
      ...
      <div class="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
        <div class="max-w-3xl">
          <div class="inline-flex items-center gap-2 rounded-full border border-[#ded3bf] bg-white/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-[rgb(var(--primary-800-rgb))] shadow-sm dark:border-[#315149] dark:bg-[#10231f]/80 dark:text-[rgb(var(--primary-200-rgb))]">
            ...
            {{ $t("gestlab.general.labels.import_certificates.module_label") }}
          </div>
          ...
          <p class="mt-3 max-w-2xl text-sm leading-6 text-[#5f6f68] dark:text-[#a9bbb4]">
            {{ $t("gestlab.general.labels.import_certificates.overview_description") }}
          </p>
        </div>
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3 xl:min-w-[32rem]">
          ...
          <div class="rounded-3xl border border-white/70 bg-white/80 p-4 shadow-sm backdrop-blur dark:border-[#315149] dark:bg-[#10231f]/80">
            <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#73827b] dark:text-[#8ea49b]">{{ $t("gestlab.general.labels.import_certificates.visible_records") }}</p>
            <p class="mt-2 text-2xl font-semibold text-[#15231f] dark:text-[#f7f1e7]">{{ visibleRecords }}</p>
          </div>
          <div class="rounded-3xl border border-white/70 bg-white/80 p-4 shadow-sm backdrop-blur dark:border-[#315149] dark:bg-[#10231f]/80">
            <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#73827b] dark:text-[#8ea49b]">{{ $t("gestlab.general.labels.import_certificates.selection") }}</p>
      ...
      <template #actions="{ data }">
        <div class="flex items-center justify-end gap-2">
          ...
          >
            ...
            {{ $t("gestlab.general.labels.import_certificates.archived") }}
          </span>
          <span
            v-else
          ...
          >
            ...
            {{ $t("gestlab.general.labels.import_certificates.certificate_status") }}
          </span>
        </div>
      </template>
    </RecordsTable>
