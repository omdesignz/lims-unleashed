<template>
  ...
  <AuthExperienceShell
    ...
    description="Prepare novos utilizadores para operar amostras, resultados, documentos, inventário e processos QMS com rastreabilidade."
    context-title="Onboarding com responsabilidade"
    context-description="A conta interna deve estar associada a perfis, permissões e responsabilidades para manter a operação auditável."
  >
    <div class="space-y-7">
      <div>
        ...
        <p class="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
          Depois do registo, configure função, departamento e permissões adequadas antes de usar em produção.
        </p>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-5">
        ...
        <label class="flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">
          ...
          <span>Confirmo que esta conta será usada conforme as políticas internas de confidencialidade, imparcialidade e segurança.</span>
        </label>
        ...
        <div class="grid gap-3 sm:grid-cols-[1fr_0.75fr]">
          ...
          <Link :href="route('login')" class="inline-flex items-center justify-center rounded-2xl border border-slate-300 px-4 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
            Já tenho conta
          </Link>
        </div>
      </form>
