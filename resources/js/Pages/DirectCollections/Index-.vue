...
const text = ref('https://www.vapsolucoes.ao')
...
const showDeleteConfirmation = ref(false);
  ...
  switch (actionId) {
    case 'delete':
      router.get(`/directcollections/destroy`, {
          recordIds: recordIds
      }, {
        preserveState: false,
        preserveScroll: true,
    ...
    case 'restore':
        router.get(`/directcollections/restore`, {
          recordIds: recordIds
        }, {
            preserveState:false,
            preserveScroll: true,
...
<records-table :record="props.record" :model="props.model" :abilities="props.abilities" :fields="props.fields" :slideOverEdit="props.slideOverEdit" :query="props.query" :actions="actions" @execute-action="($event) => {showDeleteConfirmation = true; actionId = $event}" @create-record="handleEdit" has-qr>
  <template v-slot:qr="data">
    <!-- content for the actions slot -->
    <img :src="data.data.qr" alt="QR Code" class="" />
