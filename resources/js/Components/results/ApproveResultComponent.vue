<template>
  <div class="space-y-8">
      ...
      <div class="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p class="mt-2 max-w-3xl text-sm text-slate-600 dark:text-slate-300">
            Aprovação definitiva para 
            <span v-if="record || form.sample_id" class="font-semibold text-emerald-700 dark:text-emerald-300">
              {{ record?.sample_id?.label || record?.code || form.sample_id || 'Amostra sem referência' }}
            </span>
          </p>
        </div>
        <div class="flex flex-wrap items-center gap-3">
          <span class="inline-flex items-center rounded-full bg-emerald-50 px-3 py-1 text-sm font-medium text-emerald-900 ring-1 ring-inset ring-emerald-700/10 dark:bg-emerald-500/15 dark:text-emerald-200 dark:ring-emerald-400/20">
            {{ form.results?.length || 0 }} resultados
          </span>
          <span :class="[
            'inline-flex items-center rounded-full px-3 py-1 text-sm font-medium ring-1 ring-inset',
            form.status === 'approved' ? 'bg-green-100 text-green-800 ring-green-700/10 dark:bg-green-500/15 dark:text-green-200 dark:ring-green-400/20' :
      ...
      <div class="mt-6 border-t border-slate-200/80 pt-6 dark:border-slate-800">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <div class="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-900/70">
            ...
            <p class="mt-2 text-sm font-semibold text-emerald-700 dark:text-emerald-300">{{ form.results?.length || 0 }}</p>
          </div>
          <div class="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-900/70">
        ...
        <!-- INSTRUCTIONS -->
        <div class="mt-4 rounded-2xl border border-green-200 bg-green-50 p-4 dark:border-green-500/20 dark:bg-green-500/10">
          <div class="flex items-start gap-3">
            <InformationCircleIcon class="h-5 w-5 text-green-900 mt-0.5" />
            <div class="space-y-1">
              <p class="text-sm font-medium text-green-900 dark:text-green-200">Validação Final</p>
              <p class="text-xs text-green-700 dark:text-green-300">
                Esta é a etapa final de validação. Os resultados serão marcados como definitivos após aprovação.
                Pode efectuar correcções finais, se necessário.
              </p>
            </div>
          </div>
        </div>
    ...
    <div v-if="isLoading" class="rounded-[28px] border border-slate-200 bg-white/95 p-12 text-center shadow-[0_18px_50px_-24px_rgba(15,23,42,0.28)] dark:border-slate-800 dark:bg-slate-950/85">
      ...
      <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">Preparando dados para validação final...</p>
    </div>
    ...
    <div v-else-if="!form.results || form.results.length === 0" class="rounded-[28px] border border-slate-200 bg-white/95 p-12 text-center shadow-[0_18px_50px_-24px_rgba(15,23,42,0.28)] dark:border-slate-800 dark:bg-slate-950/85">
      <ShieldCheckIcon class="mx-auto h-12 w-12 text-slate-300 dark:text-slate-700" />
      <h3 class="mt-6 text-sm font-semibold text-slate-900 dark:text-white">Nenhum resultado encontrado</h3>
      <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">Não há resultados para aprovação nesta amostra.</p>
      <div class="mt-6">
        <button 
          @click="loadResultsManually"
          type="button"
              ...
              <div class="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
                ...
                <div class="flex-1 space-y-4">
                    ...
                    <div v-if="isEditing(result)" class="space-y-3">
                      ...
                      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div v-if="result.min_ref_value || result.max_ref_value"
                          ...
                          <input 
                            :value="editedValues[getResultUniqueId(result)]?.minRef || result.min_ref_value || 0"
                            @input="handleInputChange(getResultUniqueId(result), 'min_ref_value', $event.target.value)"
                            type="number"
                            class="block w-full rounded-xl border-0 bg-white py-2 text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300 focus:ring-2 focus:ring-inset focus:ring-blue-900 dark:bg-slate-900 dark:text-white dark:ring-slate-700 dark:placeholder:text-slate-500 dark:focus:ring-emerald-400 sm:text-sm sm:leading-6"
                            placeholder="0.00"
                          />
                        ...
                        <div v-if="result.min_ref_value || result.max_ref_value"
                          ...
                          <input 
                            :value="editedValues[getResultUniqueId(result)]?.maxRef || result.max_ref_value || 0"
                            @input="handleInputChange(getResultUniqueId(result), 'max_ref_value', $event.target.value)"
                            type="number"
                            class="block w-full rounded-xl border-0 bg-white py-2 text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300 focus:ring-2 focus:ring-inset focus:ring-blue-900 dark:bg-slate-900 dark:text-white dark:ring-slate-700 dark:placeholder:text-slate-500 dark:focus:ring-emerald-400 sm:text-sm sm:leading-6"
                            placeholder="0.00"
                          />
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-slate-50/80 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/80 md:col-span-2">
                          ...
                          <div class="space-y-2">
                            <textarea 
                              v-model="result.approval_notes"
                              rows="3"
                              class="block w-full rounded-2xl border-slate-300 bg-white shadow-sm focus:border-blue-900 focus:ring-blue-900 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:placeholder:text-slate-500 dark:focus:border-emerald-400 dark:focus:ring-emerald-400 sm:text-sm"
                              placeholder="Adicione observações gerais sobre a aprovação, se necessário..."
                            />
                  ...
                  <div class="space-y-2">
                    ...
                    <!-- Approval Notes -->
                    <div v-if="result.verification_status === 'rejected' && result.approval_notes"
                      ...
                      <div class="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p class="mt-1 text-xs text-red-700 dark:text-red-300">{{ result.approval_notes }}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div class="flex flex-row gap-3 xl:ml-6 xl:min-w-[170px] xl:flex-col">
                  <!-- Approve Button -->
                  <button 
                    @click="toggleApproval(result, 'approved')"
                  ...
                  <!-- Reject Button -->
                  <button 
                    @click="toggleApproval(result, 'rejected')"
                    :disabled="isEditing(result)"
                    :class="[
                      'flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold transition-all duration-200',
        ...
        <div class="flex flex-wrap items-center gap-4">
          <div class="flex items-center gap-2">
            <div :class="[
              ...
              form.status === 'approved' ? 'bg-green-500' :
        ...
        <button 
          ...
          :class="[
            ...
            form.processing || editMode
              ...
              : 'bg-gradient-to-r from-emerald-700 via-emerald-600 to-emerald-500 text-white hover:from-emerald-600 hover:to-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-2 dark:focus:ring-emerald-400 dark:focus:ring-offset-slate-950'
          ]"
        >
          <ShieldCheckIcon class="h-5 w-5" />
          {{ form.processing ? 'A processar...' : 'Confirmar aprovação' }}
        </button>
      </div>
    </div>
  </div>
...
const initializeApproval = () => {
    if (!props.form.results || props.form.results.length === 0) {
        errorMessage.value = 'Nenhum resultado encontrado para aprovação.';
    ...
    props.form.results.forEach((result, index) => {
        ...
        // Minimum and Maximum Ref Values
        if (result.min_ref_value || result.max_ref_value) {
            result.min_ref_value = result.min_ref_value || 0;
            result.max_ref_value = result.max_ref_value || 0;
        }
    });
...
const toggleEditMode = (result) => {
    ...
    if (currentlyEditing) {
        ...
        editedValues.value[resultId] = {
            ...
            minRef: currentMinRef,
            maxRef: currentMaxRef
        };
...
const saveEdit = (resultId) => {
    ...
    // 🛑 CRITICAL FIX: ONLY update approved_value.
    // Update the result fields
    result.approved_value = newValue;
    result.uncertainty_value = newUncertainty === '' ? null : newUncertainty; // ✅ NEW: Update uncertainty
    result.min_ref_value = editedValues.value[resultId]?.minRef;
    result.max_ref_value = editedValues.value[resultId]?.maxRef;
...
const handleInputChange = (resultId, field, value) => {
    ...
    if (!editedValues.value[resultId]) {
        editedValues.value[resultId] = { value: '', uncertainty: '', minRef: 0, maxRef: 0 };
    }
    ...
    } else if (field === 'min_ref_value') {
        editedValues.value[resultId].minRef = value;
    } else if (field === 'max_ref_value') {
        editedValues.value[resultId].maxRef = value;
    }
};
...
const cancelEdit = (resultId) => {
    ...
    if (resultIndex !== -1) {
        ...
        const originalMinRef = result.min_ref_value || 0;
        const originalMaxRef = result.max_ref_value || 0;
        
        // Restore only the approved_value to the original value
        result.approved_value = originalValue;
        result.uncertainty_value = originalUncertainty; // ✅ NEW: Restore uncertainty
        result.min_ref_value = originalMinRef;
        result.max_ref_value = originalMaxRef;
...
// Toggle approval status for a specific result
const toggleApproval = (result, status) => {
    ...
    if (status === 'rejected') {
        props.form.results[resultIndex].approval_notes = 
            props.form.results[resultIndex].approval_notes || 'Resultado rejeitado na aprovação.';
    }
