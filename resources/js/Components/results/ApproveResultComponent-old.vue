...
import { CheckIcon, XMarkIcon, DocumentMagnifyingGlassIcon, PencilIcon, CalculatorIcon, ShieldCheckIcon } from "@heroicons/vue/24/outline";
...
const initializeApproval = () => {
    console.log('Initializing approval...');
    ...
    isLoading.value = false;
    errorMessage.value = '';
    console.log('Approval initialized with', props.form.results.length, 'results');
};
...
const saveEdit = (resultId) => {
    ...
    console.log('Saving edit for result:', resultId, 'New approved value:', newValue, 'Original verified value:', result.verified_value);
...
const cancelEdit = (resultId) => {
    ...
    if (resultIndex !== -1) {
        ...
        console.log('Cancelled edit for result:', resultId, 'Restored approved_value to:', originalValue);
    }
...
const toggleApproval = (result, status) => {
    ...
    console.log('Toggled approval for result:', getResultUniqueId(result), 'Status:', status);
};
...
const submitVerification = () => {
    ...
    console.log('Submitting verification data:', submissionData);
...
// Initialize on mount
onMounted(() => {
    console.log('ApproveResultComponent mounted');
    setTimeout(() => {
        initializeApproval();
...
<div class="space-y-8">
    ...
    <div v-if="isLoading" class="text-center py-12">
        ...
        <p class="mt-4 text-gray-600">Carregando resultados para aprovação...</p>
    </div>
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                ...
                                <div class="mt-2">
                                    ...
                                    <div v-if="isEditing(result)" class="space-y-3">
                                        ...
                                        <div class="flex items-center space-x-3">
                                            <input :value="editedValues[getResultUniqueId(result)]?.value || result.approved_value || result.verified_value || ''"
                                ...
                                <div v-if="result.verification_status === 'rejected' && result.approval_notes"
                                    ...
                                    <span class="ml-2 text-red-700">{{ result.approval_notes }}</span>
                                </div>
                            </div>
                            ...
                            <div class="ml-4 flex items-center space-x-2">
                                <!-- Approve button -->
                                <button @click="toggleApproval(result, 'approved')"
                                        :class="[
                                ...
                                <!-- Reject button -->
                                <button @click="toggleApproval(result, 'rejected')"
                                        :class="[
                                            'p-2 rounded-full transition-colors',
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="p-6">
                <h4 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">
                    Observações Gerais da Aprovação
                </h4>
                <textarea v-model="form.notes"
                          rows="4"
                          class="w-full border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 sm:text-sm p-3"
                          placeholder="Adicione observações gerais sobre a aprovação (opcional)..."></textarea>
    ...
    <div v-if="!isLoading && form.results && form.results.length > 0" 
        ...
        <div class="flex space-x-3">
            ...
            <button @click="submitVerification"
                    ...
                    class="px-6 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 font-medium disabled:opacity-50 disabled:cursor-not-allowed">
                {{ form.processing ? 'Processando...' : 'Confirmar Aprovação' }}
            </button>
        </div>
    </div>
</div>
