...
<template>
    ...
    <div class="border-b border-gray-200 bg-white px-4 py-5 sm:px-6">
        ...
        <div class="ml-4 mt-4">
            <h3 class="text-base font-semibold leading-6 text-gray-900">{{ $t('gestlab.general.labels.signature.title') }}</h3>
        ...
        <div class="ml-4 mt-4 flex-shrink-0">
            <span class="isolate inline-flex rounded-md shadow-sm">
                ...
                <button :disabled="!props?.currentSignature?.length" @click="$emit('delete', e)" type="button" class="relative -ml-px inline-flex items-center bg-white px-2 py-2 text-gray-500 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 focus:z-10 text-sm">
                <span class="sr-only">{{ $t('gestlab.general.buttons.delete') }}</span>
                <TrashIcon class="h-5 w-5" aria-hidden="true" /> {{ $t('gestlab.general.buttons.delete') }}
                </button>
