<template>
  <Dialog :open="isOpen" @close="close" class="relative z-50">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
      <DialogPanel class="flex h-[86vh] w-full max-w-6xl flex-col overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-2xl">
        <div class="border-b border-slate-200 bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.14),_transparent_34%),linear-gradient(135deg,_#f8fafc,_#eef6ff)] px-6 py-5">
          <div class="flex items-start justify-between gap-4">
            <div>
              <p class="text-[11px] font-semibold uppercase tracking-[0.22em] text-sky-700">Retenção e arquivo</p>
              <DialogTitle class="mt-2 text-2xl font-semibold text-slate-900">
            ...
            <button @click="close" class="rounded-2xl border border-slate-200 bg-white p-2 text-slate-500 transition hover:text-slate-700">
              <XMarkIcon class="h-6 w-6" />
            </button>
          </div>

          <div class="mt-5 grid gap-3 sm:grid-cols-3">
            <article class="rounded-2xl border border-white bg-white px-4 py-4 shadow-sm">
              <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Itens arquivados</p>
              <p class="mt-3 text-3xl font-semibold text-slate-900">{{ archivedItems.length }}</p>
            </article>
            <article class="rounded-2xl border border-white bg-white px-4 py-4 shadow-sm">
              <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Documentos</p>
              <p class="mt-3 text-3xl font-semibold text-slate-900">{{ archivedFilesCount }}</p>
            </article>
            <article class="rounded-2xl border border-white bg-white px-4 py-4 shadow-sm">
              <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Pastas</p>
              <p class="mt-3 text-3xl font-semibold text-slate-900">{{ archivedFoldersCount }}</p>
            </article>
          </div>
        </div>

        <div class="flex-1 overflow-auto bg-slate-50/60 p-6">
            ...
            >
              ...
              <dl class="mt-4 grid gap-3 sm:grid-cols-2">
                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                  <dt class="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Última alteração</dt>
                  <dd class="mt-2 text-sm font-medium text-slate-900">{{ formatDate(item.modifiedAt) }}</dd>
                </div>
                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                  <dt class="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Recuperação</dt>
                  <dd class="mt-2 text-sm font-medium text-slate-900">Disponível imediatamente</dd>
                </div>
              </dl>
          ...
          <div v-else class="flex h-full items-center justify-center">
            <div class="rounded-[1.75rem] border border-dashed border-slate-300 bg-white px-8 py-10 text-center shadow-sm">
              <p class="text-base font-semibold text-slate-900">{{ $t('gestlab.general.labels.vap_filemanager.archived_items.no_items_found') }}</p>
        ...
        <Dialog :open="showDeleteDialog" @close="showDeleteDialog = false" class="relative z-50">
          <div class="fixed inset-0 bg-slate-950/50" aria-hidden="true" />
          <div class="fixed inset-0 flex items-center justify-center p-4">
            <DialogPanel class="w-full max-w-md rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-2xl">
