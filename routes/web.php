...
Route::get('/cast', function () {
    return \App\Models\ParameterProfile::find(12);
});

Route::get('/labels', function() {

    return view('PDFs.sample_labels', [
        'qr' => \App\Models\CollectionProduct::first()->Qr
    ]);
});
...
Route::get('/', function (Request $request, GeneralSettings $settings) {
    ...
    return \Inertia\Inertia::render('Public/Landing', [
        ...
        'metrics' => Cache::remember('public-landing.metrics', now()->addMinutes(15), function () {
            return [
                'samples' => \App\Models\VAPSampleEntry::query()->count(),
                'certificates' => \App\Models\QualityCertificate::query()->count(),
                'inventory_items' => \App\Models\InventoryItem::query()->count(),
                'customer_requests' => \App\Models\CustomerRequest::query()->count(),
            ];
        }),
    ]);
})->name('landing');
...
Route::middleware(['auth', 'account.deactivated', 'verified'])->group(function () {
    ...
    Route::controller(RatingController::class)->group(function () {
        Route::get('rate/{rateableType}/{rateableId}', 'create')->name('rating.create');
        Route::post('rate/{rateableType}/{rateableId}', 'store')->name('rating.store');
    });
    ...
    Route::controller(\App\Http\Controllers\EnvironmentalConditionController::class)->group(function () {
...
Route::prefix('portal')->name('portal.')->group(function () {
    ...
    Route::controller(PortalController::class)->group(function () {
        ...
        Route::get('invoices/getInvoicePDF', 'getInvoicePDF')->name('invoices.getInvoicePDF');
        Route::get('receipts', 'receipts')->middleware('auth:portal')->name('receipts');
        Route::get('receipts/getReceiptPDF', 'getReceiptPDF')->name('receipts.getReceiptPDF');
        Route::get('contractguides', 'contractguides')->middleware('auth:portal')->name('contractguides');
        Route::get('contractguides/getContractGuidePDF', 'getContractGuidePDF')->name('contractguides.getContractGuidePDF');
        Route::get('creditnotes', 'creditnotes')->middleware('auth:portal')->name('creditnotes');
        Route::get('creditnotes/getCreditNotePDF', 'getCreditNotePDF')->name('creditnotes.getCreditNotePDF');
